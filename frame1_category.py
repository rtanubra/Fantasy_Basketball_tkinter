...
#========================== FUNCTIONS FOR CREATING HEADERS AND TABLES ============================
        ...
        #f1_t1_head
def f1_t1_head_create(frame,p1,p2,p3,row_number):
    objects = []
    ...
    return objects
    #***************Frame1 table1 body***************
def f1_t1_row_create(frame,cat,s1,s2,s3,winner,row_number):
    objects = []
...
def f1_t2_head_create(frame,row_number):
    ...
    return [head_sum_1,head_sum_2]
        ...
        #***************Frame1 table2 body***************
def f1_t2_body_create(frame,p,count,row_number):
...
#==============================FUNCTIONS TO UPDATE TABLES IN FRAME 1=======================
                ...
                #***************Frame1 table1 header***************
def f1_t1_head_update(frame,p1,p2,p3,header_obj):
    ...
    return header_obj
                ...
                #***************Frame1 table1 body***************
def f1_t1_row_update(frame,stat,s1,s2,s3,winner,body_obj):
    ...
    return body_obj
                ...
                #***************Frame1 table2 body***************
def f1_t2_body_update(frame,players,counts,body_obj):
    if len(players) == 3:
...
def initiate_frame1_on_start(
    ...
    f1_t1_head = f1_t1_head_create(frame,players[0],players[1],players[2],0)
    f1_t1_body = {}
    for i,stat in enumerate(stats_of_interest):
        stat_vals = list(cat_stat_df.loc[stat])
        if stat != "Tov":
            f1_t1_body[stat] = f1_t1_row_create(frame,stat,round(stat_vals[0],2),round(stat_vals[1],2),round(stat_vals[2],2),players[stat_vals.index(max(stat_vals))],i+1)
        else:
            f1_t1_body[stat] = f1_t1_row_create(frame,stat,round(stat_vals[0],2),round(stat_vals[1],2),round(stat_vals[2],2),players[stat_vals.index(min(stat_vals))],i+1)
    
    f1_t2_head= f1_t2_head_create(frame, 11)
    f1_t2_body = {}

    for i,player in enumerate(players):
        f1_t2_body[str(i)] = f1_t2_body_create(frame,player,cat_win_count_df.loc[player,"Category_wins"],i+12)
    """
    ...
    """
    return f1_t1_head,f1_t1_body,f1_t2_head,f1_t2_body
...
def update_frame1_3p(frame,cat_stat_df,cat_win_count_df,f1_t1_head,f1_t1_body,f1_t2_body,stats_of_interest):
    players = list(cat_stat_df.columns)
    f1_t1_head = f1_t1_head_update(frame,players[0],players[1],players[2],f1_t1_head)
    ...
    for i,stat in enumerate(stats_of_interest):
        ...
        f1_t1_body[stat] = f1_t1_row_update(frame,stat,stat_scores[0],stat_scores[1],stat_scores[2],winner,f1_t1_body[stat])
    ...
    f1_t2_body = f1_t2_body_update(frame,players,cat_scores_summary,f1_t2_body)
...
def update_frame1_2p(frame,cat_stat_df,cat_win_count_df,f1_t1_head,f1_t1_body,f1_t2_body,stats_of_interest):
    ...
    for i,stat in enumerate(stats_of_interest):
        ...
        if stat != "Tov":
            ...
            winner = players[stat_scores.index(min(stat_scores))]
        f1_t1_body[stat] = f1_t1_row_update(frame,stat,stat_scores[0],stat_scores[1],0,winner,f1_t1_body[stat])
